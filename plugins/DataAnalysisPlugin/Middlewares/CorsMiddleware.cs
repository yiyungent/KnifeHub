...
namespace DataAnalysisPlugin.Middlewares
{
    public class CorsMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext httpContext, IPluginFinder pluginFinder)
        {
            ...
            isMatch = httpContext.Request.Path.Value.StartsWith($"/api/plugins/{(nameof(DataAnalysisPlugin))}", StringComparison.CurrentCultureIgnoreCase);

            if (isMatch)
            {
                httpContext.Response.Headers.Add("access-control-allow-origin", "*");
                httpContext.Response.Headers.Add("access-control-allow-headers", "*");
                if (httpContext.Request.Method.ToLower() == "OPTIONS".ToLower())
                {
