...
namespace DataAnalysisPlugin
{
    public class DataAnalysisPlugin : BasePlugin, ITimeJobPlugin
    {
        ...
        {
            Console.WriteLine($"{nameof(DataAnalysisPlugin)}: {nameof(AfterEnable)}");
            return base.AfterEnable();
        }

        public override (bool IsSuccess, string Message) BeforeDisable()
        {
            Console.WriteLine($"{nameof(DataAnalysisPlugin)}: {nameof(BeforeDisable)}");
        ...
        public async Task ExecuteAsync()
        {
            try
            {
                var settings = PluginSettingsModelFactory.Create<SettingsModel>(nameof(DataAnalysisPlugin));

                #region 爱今天-备份记录
                try
                {
                    if (DateTime.Now - ITodaysBackupLastExecute > TimeSpan.FromSeconds(settings.ITodays.Backup.SecondsPeriod))
                    {
                        ITodaysBackupLastExecute = DateTime.Now;

                        #region 备份到 本地
                        // 备份到 本地
                        if (settings.ITodays.Backup.EnabledBackupToLocal)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    // 登录成功
                                    string folderPath = Path.Combine(PluginPathProvider.PluginsRootPath(), nameof(DataAnalysisPlugin), "ITodays", "Backups");
                                    ...
                                    #region 备份日期记录
                                    if (!string.IsNullOrEmpty(settings.ITodays.Backup?.DateRecordStartTime)
                                        &&
                                        DateTime.TryParse(settings.ITodays.Backup.DateRecordStartTime, out DateTime startTime)
                                        &&
                                        !string.IsNullOrEmpty(settings.ITodays.Backup?.DateRecordEndTime)
                                        &&
                                        DateTime.TryParse(settings.ITodays.Backup.DateRecordEndTime, out DateTime endTime))
                                    {
