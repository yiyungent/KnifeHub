...
namespace GoogleTasksPlugin.Utils
{
    ...
    {
        public static Microsoft.Extensions.Logging.ILogger<GitHubUtil> Logger { get; set; }

        public const string AppName = nameof(GoogleTasksPlugin);
        ...
        {
            #region GitHub
            GitHubClient gitHubClient = new GitHubClient(new ProductHeaderValue(AppName));
            ...
            try
            {
                var createChangeSet = gitHubClient.Repository.Content.CreateFile(owner, repo, targetFilePath,
                   new CreateFileRequest(message: $"{AppName} {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}",
                   content: fileContent, branch: branch, convertContentToBase64: convertContentToBase64))
                    .Result;
            }
            catch (Exception ex)
            {
                Logger?.LogError(ex, $"{nameof(CreateFile)}");
            }
            #endregion
        ...
        public void UpdateFile(string repoOwner, string repoName, string repoBranch, string repoTargetFilePath, string fileContent, string accessToken, bool convertContentToBase64 = true)
        {
            ...
            try
            {
                ...
                if (existingFile != null && existingFile.Count >= 1)
                {
                   ...
                   new UpdateFileRequest(message: $"{AppName} {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}",
            ...
            catch (Exception ex)
            {
               ...
               content: fileContent, branch: branch, convertContentToBase64: convertContentToBase64)).Result;
                ...
                Logger?.LogError(ex, $"{nameof(UpdateFile)}");
            }
            #endregion
        ...
        public List<string> Files(string repoOwner, string repoName, string repoBranch, string repoTargetDirPath, string accessToken)
        {
            ...
            {
                Logger?.LogError(ex, $"{nameof(Files)}");
            }
                ...
                if (existingFile != null && existingFile.Count >= 1)
                {
                    ...
                    if (!string.IsNullOrEmpty(sha))
                    {
                        gitHubClient.Repository.Content.DeleteFile(owner, repo, targetFilePath,
                            new DeleteFileRequest(message: $"{AppName} {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}",
